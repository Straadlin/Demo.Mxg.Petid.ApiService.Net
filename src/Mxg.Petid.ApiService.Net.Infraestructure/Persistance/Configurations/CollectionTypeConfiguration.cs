...
public class CollectionTypeConfiguration : IEntityTypeConfiguration<CollectionType>
{
    public void Configure(EntityTypeBuilder<CollectionType> builder)
    {
        // Fluent API
        builder.ToTable("COLLECTION_TYPE").HasKey(e => e.Id);

        builder.HasMany(c => c.Types)
               .WithOne(t => t.CollectionType)
               .HasForeignKey(t => t.CollectionTypeId);

        builder.Property(c => c.Id).HasColumnName("COLLECTION_TYPE_ID").IsRequired();
        builder.Property(c => c.Code).HasColumnName("CODE").IsRequired();
        builder.Property(c => c.Description).HasColumnName("DESCRIPTION").IsRequired(false);
